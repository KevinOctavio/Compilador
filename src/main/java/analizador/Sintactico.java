...
public class Sintactico<T> {
	...
	final int clase = 0, publico = 1, privado = 2, whilex = 3, entero = 4, booleano = 5, llaveizq = 6, llaveder = 7,
			...
			truex = 17, falsex = 18, brackizq = 19, brackder = 20, div = 21, mas = 22, menos = 23, mult = 24, ifx = 25, doble = 26,
...
//		Sx s = null;
		...
		while (type == entero || type == booleano || type == doble)
			Declaracion();
		if (this.type == whilex || this.type == ifx || this.type == entero || this.type == booleano || this.type == doble)
			Statuto();
		eat(llaveder);
...
//		System.out.println(estructura);
	...
	public void Declaracion() {
		...
		case doble:
			...
			tok = this.tok;System.out.println("Entra1|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
			eat(ID);
			
			if (type == EQ) {
				System.out.println("Entra2|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");eat(EQ);

				if (type == num) {
					System.out.println("Entra3|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");eat(num);
				}else
				if (type == numd) {
					System.out.println("Entra4|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");eat(numd);
				}
				else if (type == falsex)
					eat(falsex);
	...
	public void Statuto() {
		...
		case ifx:
			...
			while (type == whilex || type == ifx || type == ID || type == booleano || type == entero || type == doble)
